...
namespace QuanLyThuVien
{
    public partial class ThemMuonTra : Form
    {
        ...
        {
            //"SELECT SoThe from TheThuVien where NgayHetHan >= GETDATE()"
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            var r = new Database().Select(string.Format("SelectNgayHetHanTTV '" + soThe + "'"));
            DateTime theTTV = Convert.ToDateTime(r["NgayHetHan"].ToString());
            if (DateTime.Compare(theTTV,DateTime.Now) <=0)
            {
                MessageBox.Show("Bạn nên gia hạn thẻ thư viện thẻ thư viện hết hạn hoặc nốt hôm nay hết hạn");
            }
            else
            {
